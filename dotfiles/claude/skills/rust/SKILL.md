...
plugin on purpose); rely on `cargo check`/`cargo build` for diagnostics.

## Shared shape

- Tiny `src/main.rs` entry; `src/app.rs` builds `AppState` + the `Router`. Per-feature
...
- Local builds need `pkg-config` + OpenSSL headers; the Docker build supplies these.
...
- **Ports.** Every container listens on 8000 internally in dev and prod. In prod no host
  ports are published; Caddy reverse-proxies by container name on 8000.
